...
app.post("/authenticate",function(req,res){
    ...
    if(filteredusers.length!==0){
        var token = jwt.sign({username:req.body.username,password:req.body.password}, 'manasa');
...
app.use(function(req,res,next){
    var x = jwt.decode(req.cookies.token)
   // console.log(x)
    //res.redirect("/")
    if(x){
        next()
    ...
    /*if(jwt.verify(req.cookies.token,"manasa")){
        next()
...
app.get("/contactslist",function(req,res){
    ...
    res.render("contactlist",{company:'edupoly',allcontacts:contactsdata,user:"manasa"})
